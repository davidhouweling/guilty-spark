...
import { MapsCommand, FormatType, PlaylistType } from "../maps.mjs";
...
describe("All Format Types - Distribution Analysis", () => {
  ...
  const mapsCommand = new MapsCommand(services, env, generateRoundRobinMaps);
  ...
  for (const count of testCounts) {
    it(`validates HCS format distribution for ${String(count)} games`, () => {
      ...
      for (let i = 0; i < iterations; i++) {
        const result = mapsCommand.generateMaps({
          count,
