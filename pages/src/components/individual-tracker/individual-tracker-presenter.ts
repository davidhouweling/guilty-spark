...
import type { GameSelectionDialogState, IndividualTrackerSectionId, IndividualTrackerSnapshot } from "./types";
  ...
  public getActions(item: TrackerListItem): readonly TrackerRowAction[] {
    ...
    if ((item.status === "not-started" || item.status === "stopped") && item.gamertag !== "") {
      actions.push({
        ...
        onClick: (): void => {
          void this.startTracker(item.gamertag !== snapshot.xboxGamertag ? item.gamertag : undefined);
        },
      });
    }
    ...
    if (hasMultipleTrackers && !item.isLive) {
      actions.push({
        label: "Set as live",
        disabled: snapshot.busy || item.trackerId == null,
        onClick: (): void => {
          if (item.trackerId != null) {
            void this.selectLiveTracker(item.trackerId);
          }
        },
      });
    }
    ...
    if (item.trackerId != null) {
      actions.push({
        label: "View tracker",
        onClick: (): void => {
          this.assignLocation(`/individual-tracker?tracker=${item.trackerId}`);
        },
      });
    }
    ...
    if (item.status === "active") {
      actions.push({
        ...
        onClick: (): void => {
          if (item.trackerId != null) {
            void this.pauseTracker(item.trackerId);
          }
        },
      });
    }
    ...
    if (item.status === "paused") {
      actions.push({
        ...
        onClick: (): void => {
          if (item.trackerId != null) {
            void this.resumeTracker(item.trackerId);
          }
        },
      });
    }
    ...
    if (item.status === "active" || item.status === "paused") {
      actions.push({
        ...
        onClick: (): void => {
          if (item.trackerId != null) {
            void this.stopTracker(item.trackerId);
          }
        },
      });
    }
    ...
    actions.push({
      ...
      disabled: snapshot.busy || item.status !== "active" || item.trackerId == null,
      onClick: (): void => {
    ...
    if (!item.isPinned) {
      actions.push({
        ...
        onClick: (): void => {
          if (item.trackerId != null) {
            void this.deleteTracker(item.trackerId);
          }
        },
      });
    }
  ...
  private async refresh(): Promise<void> {
    ...
    try {
      ...
      const [trackerListResponse, activeStatusResponse] = await Promise.all([
        this.config.services.individualTrackerService.getTrackers(userId),
        this.config.services.individualTrackerService.getActiveTrackerState(userId),
      ]);
      ...
      this.updateSnapshot((snapshot) => ({
        ...
        userId,
        xboxGamertag: session.xboxGamertag ?? null,
