...
export class IndividualTrackerOverlayPresenter {
  static getDefaultTeamColors(): [TeamColor, TeamColor] {
    return [getTeamColorOrDefault(undefined, 0), getTeamColorOrDefault(undefined, 1)];
  }

  static getActiveSeries(timeline: readonly ViewerTimelineItem[]): ViewerSeriesTab | null {
  ...
  static buildTabs(timeline: readonly ViewerTimelineItem[]): readonly OverlayTab[] {
    let matchIdx = 0;
  ...
  static getSelectedTabIndex(tabs: readonly OverlayTab[], selectedMatchId: string | null): number {
  ...
  static buildTickerGroups(matchStatsState: MatchStatsState | null, matchIndex: number): TickerMatchGroup[] {
  ...
  static getShowTabs(renderModel: IndividualTrackerViewerRenderModel): boolean {
