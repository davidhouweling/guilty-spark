...
const extractionMatch: MatchStats<GameVariantCategory.MultiplayerExtraction> = {
  MatchId: "test-extraction-match",
  MatchInfo: {
  ...
  Teams: [
    {
      ...
      Stats: {
        ...
        ExtractionStats: {
          ...
          ExtractionInitiationsCompleted: 4,
...
} as unknown as MatchStats<GameVariantCategory.MultiplayerExtraction>;

const playerXuidsToGametags = new Map([["xuid(test-player)", "TestPlayer"]]);

describe("ExtractionMatchEmbed", () => {
  const locale = "en-US";
  let discordService: DiscordService;
  let haloService: HaloService;
  let guildConfig: GuildConfigRow;
  let matchEmbed: ExtractionMatchEmbed;

  beforeEach(() => {
    discordService = aFakeDiscordServiceWith();
    haloService = aFakeHaloServiceWith();
    guildConfig = aFakeGuildConfigRow();
    matchEmbed = new ExtractionMatchEmbed({ discordService, haloService, guildConfig, locale });
  });

  describe("getEmbed", () => {
    it("returns the expected embed", async () => {
      const result = await matchEmbed.getEmbed(extractionMatch, playerXuidsToGametags);
