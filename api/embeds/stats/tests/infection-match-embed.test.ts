...
const infectionMatch: MatchStats<GameVariantCategory.MultiplayerInfection> = {
  MatchId: "test-infection-match",
  MatchInfo: {
  ...
  Teams: [
    {
      ...
      Stats: {
        ...
        InfectionSTats: {
          ...
          RoundsSurvivedAsSpartan: 2,
          TimeAsLastSpartanStanding: "PT1M30S",
          SpartansInfected: 5,
      ...
      PlayerTeamStats: [
        {
          ...
          Stats: {
            ...
            InfectionSTats: {
              ...
              RoundsSurvivedAsSpartan: 1,
              TimeAsLastSpartanStanding: "PT45S",
              SpartansInfected: 3,
              SpartansInfectedAsAlpha: 1,
            },
          },
        },
      ],
...
} as unknown as MatchStats<GameVariantCategory.MultiplayerInfection>;

const playerXuidsToGametags = new Map([["xuid(test-player)", "TestPlayer"]]);

describe("InfectionMatchEmbed", () => {
  const locale = "en-US";
  let discordService: DiscordService;
  let haloService: HaloService;
  let guildConfig: GuildConfigRow;
  let matchEmbed: InfectionMatchEmbed;

  beforeEach(() => {
    discordService = aFakeDiscordServiceWith();
    haloService = aFakeHaloServiceWith();
    guildConfig = aFakeGuildConfigRow();
    matchEmbed = new InfectionMatchEmbed({ discordService, haloService, guildConfig, locale });
  });

  describe("getEmbed", () => {
    it("returns the expected embed", async () => {
      const result = await matchEmbed.getEmbed(infectionMatch, playerXuidsToGametags);
