...
const firefightMatch: MatchStats<GameVariantCategory.MultiplayerFirefight> = {
  MatchId: "test-firefight-match",
  MatchInfo: {
...
} as unknown as MatchStats<GameVariantCategory.MultiplayerFirefight>;

const playerXuidsToGametags = new Map([["xuid(test-player)", "TestPlayer"]]);

describe("FirefightMatchEmbed", () => {
  const locale = "en-US";
  let discordService: DiscordService;
  let haloService: HaloService;
  let guildConfig: GuildConfigRow;
  let matchEmbed: FirefightMatchEmbed;

  beforeEach(() => {
    discordService = aFakeDiscordServiceWith();
    haloService = aFakeHaloServiceWith();
    guildConfig = aFakeGuildConfigRow();
    matchEmbed = new FirefightMatchEmbed({ discordService, haloService, guildConfig, locale });
  });

  describe("getEmbed", () => {
    it("returns the expected embed", async () => {
      const result = await matchEmbed.getEmbed(firefightMatch, playerXuidsToGametags);
