...
function createHeaders(spartanToken: string, clearanceToken?: string): HeadersInit {
  return {
...
async function fetchBinary(url: string, spartanToken: string, clearanceToken: string): Promise<Uint8Array> {
  const response = await fetch(url, {
    method: "GET",
    headers: {
      ...createHeaders(spartanToken, clearanceToken),
      Accept: "*/*",
    },
  });
...
async function resolveAuthContextUncached(): Promise<AuthContext> {
  ...
  try {
    const playerScopedClearance = await fetchJson<{ FlightConfigurationId: string }>(playerScopedClearanceUrl, spartanToken);
    return {
...
function findPatternBitOffset(data: Uint8Array, startBit: number, endBitExclusive: number, pattern: Uint8Array): number | null {
...
function decodeUtf16Le(bytes: Uint8Array): string {
  return new TextDecoder("utf-16le").decode(bytes).replace(/\u0000+$/u, "").trim();
}
...
function parseHighlightEvents(chunkBytes: Uint8Array, filmMajorVersion: number): HighlightEvent[] {
  ...
  for (let markerBit = 0; markerBit <= totalBits - 8; markerBit += 1) {
    ...
    const dedupeKey = [event.xuid, event.gamertag, event.eventType, event.timeMs.toString(), event.typeHint.toString(), event.medalValue.toString()].join(":");
...
function buildPlayerValidations(matchStats: MatchStats, events: HighlightEvent[]): PlayerValidation[] {
  ...
  }).filter((player) => player.gamertag != null || player.expected.kills > 0 || player.expected.deaths > 0 || player.expected.medals > 0);
}

function buildTeamScoreTimeline(matchStats: MatchStats, events: HighlightEvent[], filmLengthMs: number): ScoreTimelinePoint[] {
...
function buildPlayerDisplayByXuid(matchStats: MatchStats, events: HighlightEvent[]): Map<string, { gamertag: string | null; teamId: number | null }> {
...
function classifyKillPair(killerTeamId: number | null, killerXuid: string, victimTeamId: number | null, victimXuid: string): KillPairing["classification"] {
...
function buildKillMatrixAnalytics(matchStats: MatchStats, kills: HighlightEvent[], deaths: HighlightEvent[], events: HighlightEvent[]): FilmTimelineOutput["analytics"]["killMatrix"] {
...
function buildPerfectCounts(matchStats: MatchStats, events: HighlightEvent[]): FilmTimelineOutput["analytics"]["perfects"] {
  const playersByXuid = buildPlayerDisplayByXuid(matchStats, events);
...
function buildLimitations(matchStats: MatchStats): string[] {
  ...
  switch (matchStats.MatchInfo.GameVariantCategory) {
    case GameVariantCategory.MultiplayerSlayer: {
      limitations.push("Score progression is derived from parsed kill events and may miss rare events that do not appear in the highlight chunk.");
      break;
    }
    case GameVariantCategory.MultiplayerCtf: {
      limitations.push("CTF progression is not reconstructed yet because flag event semantics are not decoded from mode markers.");
      break;
    }
    case GameVariantCategory.MultiplayerKingOfTheHill: {
      limitations.push("King of the Hill hill progression is reconstructed as a heuristic from mode event ticks (~5s uncontested occupation = 1 tick, 8 ticks = 1 point). Contest periods produce no ticks and are inferred from gaps.");
      break;
    }
    case GameVariantCategory.MultiplayerStrongholds: {
      limitations.push("Strongholds zone ownership and contested-state timing are not reconstructed yet from film data.");
      break;
    }
...
export async function extractFilmTimeline(matchId: string): Promise<FilmTimelineOutput> {
  ...
  const highlightChunkBytes = await fetchBinary(highlightChunkUrl, authContext.spartanToken, authContext.clearanceToken);
  ...
  return {
    ...
    match: {
      ...
      players: matchStats.Players.map((player) => ({
        ...
        teamId: player.LastTeamId,
        kills: player.PlayerTeamStats.reduce((total, playerTeamStat) => total + playerTeamStat.Stats.CoreStats.Kills, 0),
        deaths: player.PlayerTeamStats.reduce((total, playerTeamStat) => total + playerTeamStat.Stats.CoreStats.Deaths, 0),
        medals: player.PlayerTeamStats.reduce(
          (total, playerTeamStat) => total + playerTeamStat.Stats.CoreStats.Medals.reduce((medalTotal, medal) => medalTotal + medal.Count, 0),
          0,
        ),
      })),
    },
