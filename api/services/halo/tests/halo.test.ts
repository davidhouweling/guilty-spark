...
import { aFakeHaloInfiniteClient } from "../fakes/infinite-client.fake";
...
import { aFakePlayerMatchesRateLimiterWith } from "../fakes/player-matches-rate-limiter.fake";
...
describe("Halo service", () => {
  ...
  describe("getPlayerXuidsToGametags()", () => {
    ...
    it("can limit to players present at beginning", async () => {
      const match = Preconditions.checkExists(getMatchStats("d81554d7-ddfe-44da-a6cb-000000000ctf"));
      Preconditions.checkExists(match.Players[0]).ParticipationInfo.PresentAtBeginning = false;

      const result = await haloService.getPlayerXuidsToGametags(match, { presentAtBeginningOnly: true });
      expect(result).toEqual(
        new Map([
          ["0200000000000000", "gamertag0200000000000000"],
          ["0500000000000000", "gamertag0500000000000000"],
  ...
  describe("validateDiscordAssociationsFromMatches()", () => {
    it("validates discord associations from matches and updates cache", async () => {
      ...
      // Mock fuzzy match dependencies to avoid side effects
      infiniteClient.getUsers.mockResolvedValue([]);

      const upsertSpy = vi.spyOn(databaseService, "upsertDiscordAssociations");
    ...
    it("invalidates discord associations when player is not found in matches", async () => {
      ...
      const playerXuidToGametag = new Map<string, string>();
