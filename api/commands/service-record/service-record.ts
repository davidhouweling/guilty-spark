...
import { ServiceRecordEmbed } from "../../embeds/service-record/service-record-embed";
...
export class ServiceRecordCommand extends BaseCommand {
  ...
  private async getHaloServiceRecordEmbed({
    ...
    try {
      const [serviceRecord, matchCount, esraData] = await Promise.allSettled([
      ...
      const embed = new ServiceRecordEmbed(
        { discordService },
        {
          ...
          csr,
          esra: esraData.status === "fulfilled" ? esraData.value.esra : 0,
        },
      );
      return embed.embed;
    } catch (error) {
